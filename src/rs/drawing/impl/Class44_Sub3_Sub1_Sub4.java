...
public class Class44_Sub3_Sub1_Sub4 extends DrawingArea {
	...
	public Class44_Sub3_Sub1_Sub4(int i, String s, boolean flag, Class47 class47) {
		...
		aBoolean1480 = false;
		try {
			Stream class44_sub3_sub2 = new Stream(class47.method549(s + ".dat",
					null), 15787);
			Stream class44_sub3_sub2_1 = new Stream(class47.method549(
					"index.dat", null), 15787);
			byte byte0 = -1;
			class44_sub3_sub2_1.currentOffset = class44_sub3_sub2
					.getUnsignedLEShort() + 4;
			if (i != 9) {
			...
			for (int l = 0; l < 256; l++) {
				...
				int i1 = anIntArray1473[l] = class44_sub3_sub2_1
						.getUnsignedLEShort();
				int j1 = anIntArray1474[l] = class44_sub3_sub2_1
						.getUnsignedLEShort();
				...
				if (k1 == 0) {
					...
					for (int j2 = 0; j2 < i1; j2++) {
						for (int l2 = 0; l2 < j1; l2++) {
							aByteArrayArray1472[l][j2 + l2 * i1] = class44_sub3_sub2
									.get();
						}
					}
				}
			...
			signlink.reporterror("36057, " + i + ", " + s + ", " + flag + ", "
					+ class47 + ", " + runtimeexception.toString());
		}
	...
	public void method455(int i, String s, int j, int k, int l) {
		try {
			...
			signlink.reporterror("89843, " + i + ", " + s + ", " + j + ", " + k
					+ ", " + l + ", " + runtimeexception.toString());
			throw new RuntimeException();
		}
	}

	public void method456(boolean flag, int i, byte byte0, int j, String s,
			int k) {
		try {
			...
			signlink.reporterror("52266, " + flag + ", " + i + ", " + byte0
					+ ", " + j + ", " + s + ", " + k + ", "
	...
	public int method457(String s, boolean flag) {
		try {
			...
			signlink.reporterror("63390, " + s + ", " + flag + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	}
	...
	public int method458(String s, int i) {
		try {
			...
			signlink.reporterror("70316, " + s + ", " + i + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	}
	...
	public void method459(byte byte0, int i, String s, int j, int k) {
		try {
			...
			for (int l = 0; l < s.length(); l++) {
				char c = s.charAt(l);
				if (c != ' ') {
					method466(aByteArrayArray1472[c], j + anIntArray1475[c], k
							+ anIntArray1476[c], anIntArray1473[c],
							anIntArray1474[c], i);
				}
				j += anIntArray1477[c];
			}
			...
			signlink.reporterror("88673, " + byte0 + ", " + i + ", " + s + ", "
	...
	public void method460(int i, int j, byte byte0, int k, int l, String s) {
		try {
			...
			for (int j1 = 0; j1 < s.length(); j1++) {
				...
				if (c != ' ') {
					method466(
							aByteArrayArray1472[c],
							i + anIntArray1475[c],
							l
									+ anIntArray1476[c]
									+ (int) (Math.sin((double) j1 / 2D
											+ (double) j / 5D) * 5D),
							anIntArray1473[c], anIntArray1474[c], k);
				}
				i += anIntArray1477[c];
			}
			return;
		} catch (RuntimeException runtimeexception) {
			signlink.reporterror("90881, " + i + ", " + j + ", " + byte0 + ", "
					+ k + ", " + l + ", " + s + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	...
	public void method461(int i, String s, byte byte0, int j, int k, int l) {
		try {
			...
			for (int i1 = 0; i1 < s.length(); i1++) {
				...
				if (c != ' ') {
					method466(
							...
							j
									+ anIntArray1475[c]
									+ (int) (Math.sin((double) i1 / 5D
											+ (double) l / 5D) * 5D),
							k
									+ anIntArray1476[c]
									+ (int) (Math.sin((double) i1 / 3D
											+ (double) l / 5D) * 5D),
			...
			signlink.reporterror("17099, " + i + ", " + s + ", " + byte0 + ", "
					+ j + ", " + k + ", " + l + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
					...
					method466(
							...
							k
									...
									+ (int) (Math.sin((double) k1 / 1.5D
											+ (double) j) * d),
							anIntArray1473[c], anIntArray1474[c], l);
				}
				i += anIntArray1477[c];
			}
			return;
		} catch (RuntimeException runtimeexception) {
			signlink.reporterror("70626, " + s + ", " + i + ", " + j + ", " + k
					+ ", " + l + ", " + i1 + ", " + j1 + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
		...
		try {
			...
			for (int j1 = 0; j1 < s.length(); j1++) {
				if (s.charAt(j1) == '@' && j1 + 4 < s.length()
					...
					if (c != ' ') {
						if (flag) {
							method466(aByteArrayArray1472[c], l
									+ anIntArray1475[c] + 1, k
									+ anIntArray1476[c] + 1, anIntArray1473[c],
									anIntArray1474[c], 0);
						}
						method466(aByteArrayArray1472[c],
								l + anIntArray1475[c], k + anIntArray1476[c],
								anIntArray1473[c], anIntArray1474[c], i);
					}
					l += anIntArray1477[c];
				}
			...
			if (aBoolean1480) {
				DrawingArea.drawHorizontalLine(l - i1, i1, 0x800000, true, k
						+ (int) ((double) anInt1478 * 0.69999999999999996D));
				return;
			}
		} catch (RuntimeException runtimeexception) {
			signlink.reporterror("34312, " + i + ", " + j + ", " + k + ", "
					+ flag + ", " + l + ", " + s + ", "
					+ runtimeexception.toString());
			throw new RuntimeException();
		}
	...
	public void method464(String s, boolean flag, int i, int j, int k, int l,
			int i1) {
		try {
			if (j != 26617) {
			...
			for (int l1 = 0; l1 < s.length(); l1++) {
				if (s.charAt(l1) == '@' && l1 + 4 < s.length()
					...
					if (c != ' ') {
						if (flag) {
							method468(aByteArrayArray1472[c], k
									+ anIntArray1475[c] + 1, 0, i1
									+ anIntArray1476[c] + 1, anIntArray1474[c],
									192, anIntArray1473[c], false);
						}
						method468(aByteArrayArray1472[c],
								k + anIntArray1475[c], i, i1
										+ anIntArray1476[c], anIntArray1474[c],
								k1, anIntArray1473[c], false);
					}
					k += anIntArray1477[c];
			...
			signlink.reporterror("64640, " + s + ", " + flag + ", " + i + ", "
					+ j + ", " + k + ", " + l + ", " + i1 + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	...
	public int method465(String s, int i) {
		try {
			...
			signlink.reporterror("97933, " + s + ", " + i + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	}
	...
	public void method466(byte abyte0[], int i, int j, int k, int l, int i1) {
		...
		if (k <= 0 || l <= 0) {
			...
			method467(DrawingArea.pixels, abyte0, i1, i2, j1, k, l, k1, l1);
			return;
		}
	}

	public void method467(int ai[], byte abyte0[], int i, int j, int k, int l,
	...
	public void method468(byte abyte0[], int i, int j, int k, int l, int i1,
			int j1, boolean flag) {
		try {
			if (flag) {
				anInt1468 = 122;
			...
			if (k + l >= DrawingArea.bottomY) {
				l -= ((k + l) - DrawingArea.bottomY) + 1;
			}
			if (i < DrawingArea.topX) {
			...
			if (i + j1 >= DrawingArea.bottomX) {
				int i3 = ((i + j1) - DrawingArea.bottomX) + 1;
				j1 -= i3;
				i2 += i3;
				l1 += i3;
			}
			if (j1 <= 0 || l <= 0) {
				...
				method469(i2, j1, j, abyte0, false, DrawingArea.pixels, k1, j2,
						l1, i1, l);
				return;
			}
		} catch (RuntimeException runtimeexception) {
			signlink.reporterror("96459, " + abyte0 + ", " + i + ", " + j
					+ ", " + k + ", " + l + ", " + i1 + ", " + j1 + ", " + flag
					+ ", " + runtimeexception.toString());
		}
		throw new RuntimeException();
	...
	public void method469(int i, int j, int k, byte abyte0[], boolean flag,
			int ai[], int l, int i1, int j1, int k1, int l1) {
		try {
			if (flag) {
				for (int i2 = 1; i2 > 0; i2++) {
				}
			}
			k = ((k & 0xff00ff) * k1 & 0xff00ff00)
					+ ((k & 0xff00) * k1 & 0xff0000) >> 8;
			k1 = 256 - k1;
			for (int j2 = -l1; j2 < 0; j2++) {
				for (int k2 = -j; k2 < 0; k2++) {
					if (abyte0[i1++] != 0) {
						int l2 = ai[l];
						ai[l++] = (((l2 & 0xff00ff) * k1 & 0xff00ff00)
								+ ((l2 & 0xff00) * k1 & 0xff0000) >> 8)
								+ k;
					} else {
						l++;
					}
			...
			signlink.reporterror("17490, " + i + ", " + j + ", " + k + ", "
					+ abyte0 + ", " + flag + ", " + ai + ", " + l + ", " + i1
					+ ", " + j1 + ", " + k1 + ", " + l1 + ", "
					+ runtimeexception.toString());
		}
