...
public class Class44_Sub3_Sub1_Sub3 extends DrawingArea {
	...
	public Class44_Sub3_Sub1_Sub3(Class47 class47, String s, int i) {
		...
		Stream class44_sub3_sub2 = new Stream(class47.method549(s + ".dat",
				null), 15787);
		Stream class44_sub3_sub2_1 = new Stream(class47.method549("index.dat",
				null), 15787);
		...
		for (int l = 0; l < i; l++) {
			...
			class44_sub3_sub2.currentOffset += class44_sub3_sub2_1
					.getUnsignedLEShort()
					* class44_sub3_sub2_1.getUnsignedLEShort();
		...
		try {
			...
			for (int k = 0; k < anInt1461; k++) {
				for (int l = 0; l < imageWidth; l++) {
					abyte0[(l + anInt1462 >> 1) + (k + anInt1463 >> 1)
							* libWidth] = imagePixels[j++];
				}
			}
			imagePixels = abyte0;
			imageWidth = libWidth;
	...
	public void method452(int i, byte byte0, int j, int k) {
		try {
			...
			signlink.reporterror("28455, " + i + ", " + byte0 + ", " + j + ", "
	...
	public void method453(int i, byte byte0, int j) {
		try {
			...
			if (j1 <= 0 || i1 <= 0) {
				...
				method454(i1, anIntArray1459, k, DrawingArea.pixels, l1, k1,
						(byte) 34, imagePixels, j1, l);
				return;
			}
		} catch (RuntimeException runtimeexception) {
			signlink.reporterror("29131, " + i + ", " + byte0 + ", " + j + ", "
					+ runtimeexception.toString());
		}
		throw new RuntimeException();
	}

	public void method454(int i, int ai[], int j, int ai1[], int k, int l,
			byte byte0, byte abyte0[], int i1, int j1) {
		try {
			int k1 = -(i1 >> 2);
			i1 = -(i1 & 3);
			...
			signlink.reporterror("42705, " + i + ", " + ai + ", " + j + ", "
					+ ai1 + ", " + k + ", " + l + ", " + byte0 + ", " + abyte0
					+ ", " + i1 + ", " + j1 + ", "
					+ runtimeexception.toString());
			throw new RuntimeException();
		}
